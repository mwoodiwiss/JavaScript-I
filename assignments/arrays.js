...
let lastCar = 0;
console.log('Last car in inventory is a ' + inventory[inventory.length - 1].car_make  + ' ' + inventory[inventory.length - 1].car_model);
...
// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older
// than the year 2000. Using the carYears array you just created, 
// find out how many cars were made before the year 2000 by populating
// the array oldCars and logging it's length.
let oldCars = [];

function getsOldCars(year) {
...
console.log(getsOldCars(2000)); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];

function getsMakes(make1, make2) {
    for (let i = 0; i < inventory.length; i++) {
        if (inventory[i].car_make === make1) {
            BMWAndAudi.push(inventory[i]);
        }

        if (inventory[i].car_make === make2) {
            BMWAndAudi.push(inventory[i]);
        }
    }
    return BMWAndAudi;
}
console.log(JSON.stringify(getsMakes('BMW', 'Audi')));
